...
Node* REG::newNode(population data)
{
    ...
    newNode->intKey = 0;
    newNode->data_count = data.period;
...
Node* REG::search(Node* parent, std::string RegionKey, int PeriodKey){ //Births oriented
...
Node* COUNT::search(Node* parent, int CountKey, string  RegionKey){ //Births oriented
    while(parent != nullptr)
    {
        if(CountKey > parent->intKey ){ // key > parent->key
            parent = parent->right_child;
        }
        else if(CountKey < parent->intKey ){ // key < parent->key
            parent = parent->left_child;
        }
        else
        {
            REG reg;
            Node* temporal = reg.searchSingular(parent->node_data_births, RegionKey);
            ...
            parent->key = temporal->key;
            parent->data_count = temporal->data_count;
            return parent;
        }
...
void COUNT::editSelect(Node* parent, int CountKey, string RegionKey, int newPeriod){
    while(parent != nullptr)
    {
        if(CountKey > parent->intKey ){ // key > parent->key
            parent = parent->right_child;
